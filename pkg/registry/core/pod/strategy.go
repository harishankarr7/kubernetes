...
func defaultPodLevelLimits(pod *api.Pod) {
	...
	for resName := range candidates {
		...
		// pod-level limit to match the pod-level request. This preserves validity
		// (requests <= limits) for pods that had pod-level requests > container limits
		// without explicit pod-level limits, which is a valid configuration.
		if podReq, hasReq := pod.Spec.Resources.Requests[resName]; hasReq {
			if podReq.Cmp(val) > 0 {
				val = podReq.DeepCopy()
			}
		}
